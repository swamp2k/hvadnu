...
function ResultView({ result }: { result: MessageAnalysisResult }) {
  ...
  return (
    <section className="result-stack" aria-live="polite">
      ...
      <article className="card uncertainty-card"><div className="section-title-row"><h3>Usikkerhed</h3><span className="uncertainty-label">{result.uncertainty.level === 'high' ? 'Høj' : result.uncertainty.level === 'medium' ? 'Middel' : 'Lav'}</span></div><BulletList items={result.uncertainty.missing} emptyText="Ingen væsentlige mangler registreret." />{result.reviewPlan.humanReviewRecommended && <div className="review-strip"><strong>Menneskelig juridisk vurdering anbefales</strong></div>}</article>
      {result.citations.length > 0 && <details className="card source-details"><summary>Kilder ({result.citations.length})</summary><div className="source-detail-list">{result.citations.map((citation) => <div key={`${citation.sourceId}-${citation.locator ?? ''}`}><strong>{citation.label}</strong><p className="muted">Status: {citation.status}{citation.locator ? ` · ${citation.locator}` : ''}</p></div>)}</div></details>}
    </section>
  );
}
...
function History({ entries }: { entries: MessageHistoryEntry[] }) {
  ...
  return (
    <section className="case-section">
      <div><p className="eyebrow">Historik</p><h2>Tidligere beskedanalyser</h2></div>
      <div className="state-list">
        {entries.map((entry) => <details className="card source-details" key={entry.id}>
          <summary>{new Date(entry.createdAt).toLocaleString('da-DK')} · {entry.analysis.summary}</summary>
          <div className="source-detail-list">
            <div><strong>Modtaget besked</strong><p>{entry.message}</p></div>
            <div><strong>Svarforslag</strong><p>{entry.analysis.suggestedReply}</p></div>
          </div>
        </details>)}
      </div>
    </section>
...
export function MessageAssistantView() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setResult(null);
      setError(cause instanceof MessageApiError && cause.status === 401 ? 'Cloudflare Access-sessionen kunne ikke godkendes.' : 'Beskeden kunne ikke analyseres lige nu.');
    } finally {
      setWorking(false);
    }
  }
  ...
  return (
    <>
      <section className="intro"><h2>Hvad har du fået?</h2><p>Indsæt beskeden. Analysen køres privat via Sonnet og gemmes automatisk i sagens historik sammen med beskeden og svarforslaget.</p></section>
      ...
      {result ? <ResultView result={result} /> : !error && <section className="empty-state"><div className="empty-icon">≡</div><h3>Analysen bliver kort og kildebaseret</h3><p>Du får: hvad der kræver svar, hvad der kan ignoreres, sagskontekst, juridisk usikkerhed og et neutralt svarforslag.</p></section>}
